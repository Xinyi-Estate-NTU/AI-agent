...
def parse_query_to_json(llm_service, query_text: str) -> Dict[str, Any]:
    ...
    response_schemas = [
        ResponseSchema(**schema_config) for schema_config in RESPONSE_SCHEMAS_CONFIG
    ]
    ...
    try:
        ...
        # 處理時間範圍 (可能以字符串形式返回)
        if parsed_output.get("時間範圍") and parsed_output["時間範圍"] not in [
            None,
            "null",
        ]:
            if isinstance(parsed_output["時間範圍"], str):
                # 處理JSON格式時間範圍
                if parsed_output["時間範圍"].startswith("{") and parsed_output[
                    "時間範圍"
                ].endswith("}"):
                    try:
                        ...
                        logger.warning(
                            f"無法解析JSON時間範圍: {parsed_output['時間範圍']}"
                        )
