...
class RealEstateAnalyzer:
    ...
    def calculate_average_price(
        ...
        """計算平均房價，增強版添加更多統計指標。"""
        logger.info(
            f"計算平均房價，行政區: {district if district else '全部'}, 額外過濾: {filters}"
        )
        ...
        if len(df) > 0:
            ...
            if district is None and count > 10:
                try:
                    district_avg = (
                        df.groupby("鄉鎮市區")["每坪單價"].mean().reset_index()
                    )
    ...
    @staticmethod
    def get_district_price_ranking(
        df: pd.DataFrame, city: str = None, top_n: int = 10
    ) -> Dict[str, Any]:
        """獲取行政區平均房價排名。"""
        ...
        try:
            ...
            # 計算各區域平均價格
            district_avg = (
                df.groupby("鄉鎮市區")["每坪單價"].agg(["mean", "count"]).reset_index()
            )
            ...
            return {"success": True, "result": result_text, "dataframe": top_districts}
    ...
    def generate_price_trend_chart(
        ...
        """
        logger.info(
            f"開始生成房價趨勢圖，城市: {city}, 區域: {district}, 圖表類型: {chart_type}"
        )
        ...
        try:
            ...
            # 2. 將交易日期轉換為日期格式
            analysis_df["交易日期"] = pd.to_datetime(
                analysis_df["交易年月日"], errors="coerce"
            )
            ...
            if is_single_year:
                ...
                time_grouped = (
                    analysis_df.groupby(["年份", "月份"])
                    .agg({"每坪單價": "mean", "交易年月日": "count"})
                    .reset_index()
                )

                # 格式化月份標籤
                time_grouped["時間標籤"] = time_grouped["月份"].map(
                    lambda m: f"{int(m)}月"
                )
                ...
                # 使用agg()進行分組彙總
                time_grouped = (
                    analysis_df.groupby("年份")
                    .agg({"每坪單價": "mean", "交易年月日": "count"})
                    .reset_index()
                )

                # 格式化年份標籤
                time_grouped["時間標籤"] = time_grouped["年份"].map(
                    lambda y: f"{int(y)}年"
                )
            ...
            try:
                ...
                fig, ax = plt.subplots(
                    figsize=(12, 6), constrained_layout=True, dpi=100
                )
                ...
                ax.set_xticklabels(
                    time_grouped["時間標籤"],
                    rotation=45 if len(time_grouped) > 6 else 0,
                )

                # 13. 共用的圖表設置，使用matplotlib的最佳實踐
                ax.set_title(
                    f"{area_text}房價趨勢圖 ({time_range_text})", fontsize=16, pad=15
                )
    ...
    def filter_data_by_attributes(
            ...
            if attr == "時間範圍" and isinstance(value, dict):
                ...
                if "交易年度" in df.columns:
                    ...
                    if start_year and end_year:
                        year_filter = (df["交易年度"] >= start_year) & (
                            df["交易年度"] <= end_year
                        )
        ...
        if filter_conditions:
            ...
            logger.info(
                f"應用 {len(filter_conditions)} 個過濾條件後數據量: {len(filtered_df)}"
            )
            return filtered_df
        else:
